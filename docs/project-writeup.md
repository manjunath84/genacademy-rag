...
> GenAcademy RAG is a course-material assistant that answers with line-level citations and refuses unsupported questions. It uses hybrid retrieval, a pinned eval corpus, Nebius generation, Pinecone serving vectors, admin document management, and a Docker Hugging Face Space deployment. The project is scale-ready by seams and evaluation discipline, not overbuilt with premature distributed infrastructure.
